...
class CryptoEnv(gym.Env):
    def __init__(self, df, df_stat, algo, timesteps, idx = 0, lookback_window_size = 50, 
                 commission = 0.00075,
        ...
        self.observation_space = spaces.Box(low = 0, high = 1, shape = (27, lookback_window_size + 1), dtype = np.float16)
    ...
    def reset(self):
        ...
        if len(self.trades) > 0:
            dict_to_csv(self.trades, 'trades', f'models/{self.algo}/Timesteps_{self.timesteps}/{self.idx}/Timesteps_{self.timesteps}_{self.reset_count}_')
    ...
    def render(self, mode='human'):
        lists_to_csv('performance', f'models/{self.algo}/Timesteps_{self.timesteps}/{self.idx}/Timesteps_{self.timesteps}_', 
        ...
        save_plot(self.graph_p_profit, self.graph_p_benchmark_profit, self.graph_overall_reward,
                            'performance', f'models/{self.algo}/Timesteps_{self.timesteps}/{self.idx}/Timesteps_{self.timesteps}_')

...
class CryptoEnv(gym.Env):
    def __init__(self, df, df_stat, algo, timesteps, lookback_window_size = 50,
        ...
        self.observation_space = spaces.Box(low = 0, high = 1, shape = (28, lookback_window_size + 1), dtype = np.float16)
    ...
    def _next_observation(self):
        ...
        obs = np.array([
            ...
            self.active_df['Moving_avg_close'].values[self.current_step:self.end_active_df],
        ...
        # get the moving average the same size as the other data
        self.mov_avg_df = self.active_df['Moving_avg_close'].values[self.current_step:self.end_active_df]
        return obs
    ...
    def _take_action(self, action):
        ...
        # get moving average at current step
        mov_avg = self.mov_avg_df[-1]

        # Get positive reward, when holding crypto and uptrend
        if crypto_holding_previous > 0 and self.original_price >= mov_avg:
            self.trend_reward = 2
        # Get positive reward, when NOT holding crypto and downtrend
        elif crypto_holding_previous <= 0 and self.original_price <= mov_avg:
            self.trend_reward = 1
        # Get negative reward, when NOT holding crypto and uptrend
        elif crypto_holding_previous <= 0 and self.original_price > mov_avg:
            self.trend_reward = 1
        # Get negative reward, when holding crypto and downtrend
        elif crypto_holding_previous > 0 and self.original_price < mov_avg:
            self.trend_reward = 2

        # get extra reward, when trade proft (sale) is positive (sold with profit)
        if action_type == 1:
            if self.trade_profit > 0:
                self.extra_reward = 0.5
            else:
                self.extra_reward = -0.5

        # reward of zero if action is buy
        if action_type == 0:
            self.trend_reward = 0

        # Empty Buy/Sell order - negative reward
        # Preventing purchases or sales that cannot be executed
        if self.min_trade_penalty == True:
            self.trend_reward = -1
        ...
        self.overall_reward += self.trend_reward + self.extra_reward
    ...
    def step(self, action, end=True):
        ...
        reward = self.trend_reward + self.extra_reward

...
def main():
    ...
    # algo_test = "PPO_env_V36_pred_21" # folder name to save the predictions on the year 2021
    algo_test = "PPO_env_V36_pred_bench" # folder name to save the predictions on the benchmark period
    TIMESTEPS = 500000 # Timesteps used during training

    models_dir_train = f"models/{algo_train}/Timesteps_{TIMESTEPS}" # path to get saved model

    models_dir_test = f"models/{algo_test}/Timesteps_{TIMESTEPS}" # path to save predictions
    log_dir_test = f"models/{algo_test}/Timesteps_{TIMESTEPS}/logs" # path to save logs of predictions (optional)
    ...
    env_eval = DummyVecEnv([lambda: CryptoEnv(df_test, df_test_stat, algo_test, TIMESTEPS, serial=True)])
